...
class DoadorModel {
    ...
    // Construtor
    constructor(doadorId, doadorNome, doadorCPF, doadorRG, doadorSexo, doadorEmail, doadorSenha, doadorTelefone, doadorEndereco, doadorCEP) {
        this.#doadorId = doadorId;
        this.#doadorNome = doadorNome;
        this.#doadorCPF = doadorCPF;
        this.#doadorRG = doadorRG;
    ...
    async listar() {
        ...
        for(let i = 0; i < rows.length; i++) {
            lista.push(new DoadorModel(rows[i]["id_doador"], rows[i]["nome"], rows[i]["CPF"], rows[i]["RG"],
                rows[i]["sexo"], rows[i]["email"], rows[i]["senha"], rows[i]["telefone"], rows[i]["endereco"], rows[i]["CEP"], 
            ));
        }
        return lista;
    }
